...
import { buildSessionCookieOptions } from '../lib/server/session';
...
test('logout cookies clear the session immediately', () => {
  const options = buildSessionCookieOptions(new Request('https://app.decisive.coach/logout'), { value: '', maxAge: 0 });
