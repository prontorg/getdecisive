...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.match(source, /Cut load/i);
  ...
  assert.match(calendarStyles, /training-plan-inline-menu__reconcile-row/i);
  ...
  assert.doesNotMatch(source, /<strong>Confidence<\/strong>/i);
  ...
  assert.match(source, /<button type="submit" className="button-secondary button-link">Cut load<\/button>/i);
  assert.match(source, /<button type="submit" className="button-secondary button-link">Use freshness<\/button>/i);
