...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.match(calendarSource, /const skipActionState = quickActionState\(workout, 'skip'\)/i);
  assert.match(calendarSource, /const supportActionState = quickActionState\(workout, 'replace_with_support'\)/i);
  assert.match(calendarSource, /const doneModifiedActionState = quickActionState\(workout, 'mark_done_modified'\)/i);
  assert.match(calendarSource, /mutateWorkout\(workout\.id, action\)/i);
  ...
  assert.match(calendarSource, /\{ action: 'skip' as const, label: 'Skip', state: skipActionState \}/i);
  assert.match(calendarSource, /\{ action: 'replace_with_support' as const, label: 'Support', state: supportActionState \}/i);
  assert.match(calendarSource, /\{ action: 'mark_done_modified' as const, label: 'Done\*', state: doneModifiedActionState \}/i);
  assert.match(calendarSource, /quickActionState/i);
  assert.match(calendarSource, /quickActionDisabled/i);
  assert.match(calendarSource, /disabled=\{quickActionDisabled\}/i);
  ...
  assert.match(calendarSource, /onClick=\{\(\) => setMenuActionByWorkout/i);
