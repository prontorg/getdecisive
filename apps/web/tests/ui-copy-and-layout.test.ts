...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.match(source, /heroTitle = isCalendarMode \? 'Calendar' : 'Plan'/i);
  assert.match(source, /Plan builder flow/i);
  ...
  assert.match(source, /Tell the planner what this month should achieve/i);
  assert.match(source, /Live week first\. Future weeks stay editable\./i);
  ...
  assert.match(source, /Build the month in 3 simple steps/i);
  assert.match(source, /Current draft status/i);
  assert.match(source, /Step 1/i);
  assert.match(source, /Step 2/i);
  assert.match(source, /Step 3/i);
  assert.match(source, /Check today before you plan the month/i);
  assert.match(source, /Tell the planner what this month should achieve/i);
  assert.match(source, /This saves your inputs, builds the next 4 weeks, and opens Step 3 below\./i);
  assert.match(source, /1\. Check the live week/i);
  assert.match(source, /2\. Adjust the draft bridge/i);
  assert.match(source, /3\. Tidy the month/i);
  ...
  assert.match(calendarStyles, /training-plan-flow-shell/i);
  assert.match(calendarStyles, /training-plan-step-grid/i);
  assert.match(calendarStyles, /training-plan-step-section/i);
