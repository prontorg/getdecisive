...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.doesNotMatch(source, /What should actually happen/i);
  ...
  assert.doesNotMatch(source, /<button type="submit">Repair<\/button>/i);
  assert.doesNotMatch(source, /<button type="submit">Cut load<\/button>/i);
  assert.doesNotMatch(source, /<button type="submit">Use freshness<\/button>/i);
  ...
  assert.doesNotMatch(source, /Next key day/i);
  assert.doesNotMatch(source, /Risk:/i);
  assert.doesNotMatch(source, /\/api\/planner\/month\/replan/i);
