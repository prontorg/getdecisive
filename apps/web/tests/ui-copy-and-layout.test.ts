...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.match(source, /What should this month become\?/i);
  assert.match(source, /Live week first\. Future weeks stay editable\./i);
  ...
  assert.match(source, /What should this month become\?/i);
  assert.match(source, /Set the key constraints and build the draft/i);
  assert.match(source, /Review the live week and the generated month/i);
  assert.match(source, /This saves your inputs, builds the next 4 weeks, and refreshes the review section below\./i);
  ...
  assert.match(calendarStyles, /planning-workspace-shell/i);
  assert.match(calendarStyles, /planning-workspace-banner/i);
