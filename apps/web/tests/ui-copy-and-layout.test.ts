...
test('training plan page uses the latest decisive monthly-planner framing and layout', async () => {
  ...
  assert.match(source, /What should actually happen this week/i);
  ...
  assert.match(calendarSource, /weekActionButtonLabel/i);
  assert.match(calendarSource, /WEEK_ACTIONS/i);
  assert.match(calendarSource, /previewWeekAction/i);
  assert.match(calendarSource, /applyWeekAction/i);
  assert.match(calendarSource, /intent: 'preview'/i);
  assert.match(calendarSource, /intent: 'apply'/i);
  assert.match(calendarSource, /Reduce load|Sharpen|Lighter weekend|Regenerate/i);
  assert.match(calendarSource, /Apply week change/i);
  assert.match(calendarSource, /keyProtectionSummary/i);
  assert.match(calendarSource, /freshnessSummary/i);
  assert.match(calendarSource, /Before structure:/i);
  assert.match(calendarSource, /After structure:/i);
  assert.match(calendarSource, /Before intent:/i);
  assert.match(calendarSource, /After intent:/i);
  ...
  assert.match(calendarSource, /completedCount/i);
  assert.match(calendarSource, /plannedCount/i);
  assert.match(calendarSource, /Completed • \{completedCount\}/i);
  assert.match(calendarSource, /Planned • \{plannedCount\}/i);
  ...
  assert.doesNotMatch(calendarSource, /training-plan-session-card__quick-actions/i);
  assert.doesNotMatch(calendarSource, /quickActionState/i);
  assert.doesNotMatch(calendarSource, /quickActionDisabled/i);
  ...
  assert.doesNotMatch(calendarStyles, /training-plan-session-card__quick-actions/i);
  assert.doesNotMatch(calendarStyles, /training-plan-session-card__quick-action-disabled/i);
  assert.doesNotMatch(calendarStyles, /training-plan-session-card__quick-action-active/i);
  ...
  assert.match(calendarSource, /const WORKOUT_ACTIONS = \['move_day', 'skip', 'replace_with_support', 'mark_done_modified', 'remove'\] as const;/i);
  ...
  assert.match(statefulBuilderSource, /Active constraints/i);
  assert.match(statefulBuilderSource, /Saved|Unsaved changes/i);
  assert.match(statefulBuilderSource, /Draft updated/i);
